...
interface Props {
  ...
  proteins: ProteinRow[];
  onClose: () => void;
  /** Optional JSX rendered in the header next to the subtitle */
  headerExtra?: ReactNode;
  /** Message shown when proteins array is empty (overrides default) */
  emptyFallback?: string;
}

export function ProteinListModal({ title, subtitle, proteins, onClose, headerExtra, emptyFallback }: Props) {
  useEffect(() => {
    ...
    >
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4 flex flex-col max-h-[80vh]">
        ...
        <div className="overflow-y-auto flex-1">
          {proteins.length === 0 ? (
            <p className="text-center text-gray-400 py-10 text-sm px-6">
              {emptyFallback ?? "No proteins found for this selection."}
            </p>
          ) : (
            <table className="w-full text-sm">
              ...
              <tbody>
                {proteins.map((p, i) => (
                  <tr key={p.accession || i} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 font-semibold text-gray-800">{p.gene_name || "—"}</td>
                    <td className="px-4 py-2 text-gray-600 text-xs leading-snug">{p.protein_name || "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="px-5 py-3 border-t border-gray-100 flex justify-between items-center text-xs text-gray-400">
          ...
          <button onClick={onClose} className="text-primary-600 hover:underline">Close</button>
        </div>
      </div>
    </div>
  );
