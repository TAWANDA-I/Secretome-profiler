...
async def _execute_pipeline(job_id: str) -> None:
    async with AsyncSessionLocal() as s:
        r = await s.execute(select(Job).where(Job.id == uuid.UUID(job_id)))
        job = r.scalar_one()
        proteins: list[str] = job.proteins
    ...
    if "hpa" in modules:
        parallel.append(_run_hpa(job_id, proteins))
...
async def _run_hpa(job_id: str, proteins: list[str]) -> None:
    await _set_module_progress(job_id, "hpa", "running", 0)
    try:
        data = await hpa_svc.fetch_concentrations(proteins)
